...
public class LabFourGame extends Game {
	...
	//bad dudes
	Sprite piranha1 = new Sprite("piranha1", "piranha1.png", null, new Point(350,350));
	Sprite piranha2 = new Sprite("piranha2", "piranha2.png", null, new Point(750,350));
	...
	public LabFourGame() {
		...
		score = 1000;
		pipe.setFixed(true);
		piranha1.setFixed(true);
		piranha2.setFixed(true);
		mario.setFixed(false);
		mario.setDeceleration(20);
	...
	/* Same, just check for null in case a frame gets thrown in before Mario is initialized */
		...
		if (mario.collidesWith(piranha1)==true && (!mario.isMoving())) {
			boolean left = mario.leftOf(piranha1);
			int direction;
			if (left) {
				direction = -1;
			} else {
			...
			mario.applyForce(100, direction);
			mario.setHitArea();
		}
		if ((mario.collidesWith(piranha2) == true) && (!mario.isMoving())) {
			boolean left = mario.leftOf(piranha2);
			int direction;
			if (left) {
				direction = -1;
			} else {
