...
public class DisplayObject{
	...
	public void updateVelocity() {
		int newVelocity = (Math.abs(this.velocity) - this.deceleration);
		if (newVelocity < 0) {
			this.velocity = 0;
		} else {
			this.setVelocity(newVelocity * (int) Math.signum(this.velocity));
		}
	}

	public boolean leftOf(DisplayObject other) {
	...
	public void movePosition() {
		...
		System.out.println("Velocity when applied");
		int velocity = this.getVelocity();
		System.out.println(velocity);
		this.setPosition(new Point(current.x + velocity, current.y));
	}
	...
	public void applyForce(int amount, int direction) {
		System.out.println("velocity when applying force");
		this.setVelocity(amount * direction);
...
//	}
	...
	public void draw(Graphics g) {
		...
		if (displayImage != null && getVisible()){
			...
			if (!this.getFixed() && this.isMoving()) {
				this.updateVelocity();
